...
def profile():
    ...
    # Safety Check: Pehle automatic check kar lo ki naye columns table me hain ya nahi, nahi toh add kar do
    extra_cols = [
        ("age", "TEXT"), ("gender", "TEXT"), ("height", "TEXT"), ("weight", "TEXT"), ("location", "TEXT"),
        ("branch", "TEXT"), ("year", "TEXT"), ("semester", "TEXT"),
        ("sleep_goal", "TEXT"), ("water_goal", "TEXT"), ("steps_goal", "TEXT"), ("screen_goal", "TEXT"), ("stress_goal", "TEXT")
    ]
    ...
    if row:
        profile_details = {
            "username": username,
            "fullname": row[0] if row[0] else "",
            "email": row[1] if row[1] else "",
            "phone": row[2] if row[2] else "",
            "age": row[3] if row[3] else "",
            "gender": row[4] if row[4] else "",
            "height": row[5] if row[5] else "",
            "weight": row[6] if row[6] else "",
            "location": row[7] if row[7] else "",
            "branch": row[8] if row[8] else "",
            "year": row[9] if row[9] else "",
            "semester": row[10] if row[10] else "",
            "sleep_goal": row[11] if row[11] else "",
...
def update_profile():
    ...
    try:
        ...
        # 3. Database connection start karo
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
        
        # 4. Safer columns update structure query
        cursor.execute("""
            UPDATE users 
            SET fullname = ?, email = ?, phone = ?, age = ?, gender = ?, height = ?, weight = ?, location = ?
            WHERE username = ?
        """, (fullname, email, phone, age, gender, height, weight, location, username))
        
        conn.commit()
        conn.close()
        
        print(f"[SUCCESS]: Profile updated successfully for user: {username}")
        return redirect("/profile")

    except Exception as e:
        # 🌟 Agar abhi bhi koi galti hogi, toh yeh terminal par exact line error print karega
...
def update_academic():
    ...
    try:
        ...
        # Database updates matching profile inputs
        cursor.execute("""
            UPDATE users 
            SET branch = ?, year = ?, semester = ?
            WHERE username = ?
        """, (branch, year, semester, username))
        
        conn.commit()
        conn.close()
        
        print(f"[SUCCESS]: Academic info updated for user: {username}")
        return redirect("/profile")

    except Exception as e:
        print(f"[CRITICAL ACADEMIC UPDATE ERROR]: {str(e)}")
        return f"Academic Database Error: {str(e)}", 500
...
def update_goals():
    ...
    try:
        ...
        screen_goal = request.form.get("screen_goal", "")
        stress_goal = request.form.get("stress_goal", "")

        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()

        # Goals schema columns updates
        cursor.execute("""
            UPDATE users 
            SET sleep_goal = ?, water_goal = ?, steps_goal = ?, screen_goal = ?, stress_goal = ?
            WHERE username = ?
        """, (sleep_goal, water_goal, steps_goal, screen_goal, stress_goal, username))
        
        conn.commit()
        conn.close()
        
        print(f"[SUCCESS]: Wellness goals updated for user: {username}")
        return redirect("/profile")
